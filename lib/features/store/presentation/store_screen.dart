...
class StoreScreen extends ConsumerWidget {
  ...
  void _buyItem(BuildContext context, WidgetRef ref, String uid, CosmeticItemEntity item) {
    ref.read(economyControllerProvider.notifier).buyCosmetic(
      uid: uid,
      cosmeticId: item.id,
      price: item.price,
    ).then((_) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Hata: Yetersiz Puan veya Zaten Sahipsin!'),
  ...
  void _equipItem(BuildContext context, WidgetRef ref, String uid, CosmeticItemEntity item) {
    if (item.type == 'frame') {
       ref.read(economyControllerProvider.notifier).setActiveFrame(uid: uid, cosmeticId: item.id);
    } else {
       ref.read(economyControllerProvider.notifier).setActiveTitle(uid: uid, cosmeticId: item.id);
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
    ...
    return Scaffold(
      ...
      body: GradientContainer(
        child: cosmeticsAsync.when(
          ...
          error: (err, stack) => Center(child: Text('Mağaza yüklenemedi: $err', style: TextStyle(color: AppColors.voteNegative))),
          data: (items) {
             ...
             return GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final itemColor = item.type == 'frame' ? AppColors.fire : Colors.amber;
                final isEquipped = (item.type == 'frame' && profile?.activeFrame == item.id) || 
                                   (item.type == 'title' && profile?.activeTitle == item.id);

                return Card(
                  color: isEquipped ? AppColors.primary.withValues(alpha: 0.2) : AppColors.surfaceElevated,
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(16),
                    side: BorderSide(
                      color: isEquipped ? AppColors.primary : itemColor.withValues(alpha: 0.3), 
                      width: 2
                    ),
                  ),
                  child: Column(
                      ...
                      if (isOwned)
                        ElevatedButton(
                          style: ElevatedButton.styleFrom(
                            backgroundColor: isEquipped ? Colors.white24 : AppColors.primary,
                            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                          ),
                          onPressed: () => _equipItem(context, ref, user.uid, item),
                          child: Text(isEquipped ? 'Kuşanıldı' : 'Kuşan', style: TextStyle(color: Colors.white)),
                        )
                      else
                        ElevatedButton.icon(
                          style: ElevatedButton.styleFrom(
                          ...
                          onPressed: () => _buyItem(context, ref, user.uid, item),
