...
class UserEntity {
  ...
  String get calculatedRank {
    ...
    // Oyun Kurucu (Özel Görevler)
    final customTasks = stats['custom_tasks'] ?? 0;
    ...
    // Rütbe Kararı
    if (totalLevels >= 18) return 'Sosyal Efsane';
    if (totalLevels >= 13) return 'Mekanın Sahibi';
    if (totalLevels >= 7) return 'Parti Yıldızı';
    if (totalLevels >= 3) return 'Ortam Şenlendirici';
    return 'Utangaç (Çaylak)';
  }
